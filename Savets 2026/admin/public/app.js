...
// A doubles entry holds both partners in the one name field, as the entry form captures
// them ("John Smith / Jane Doe"). "+" is never treated as a separator — it belongs to the
// age categories.
function splitDoublesPartners(name) {
  return String(name || "")
...
function renderEventRankings() {
  ...
  const withRanks = entriesForEvent
    ...
    .sort((a, b) => {
      if (a.bestRank === null && b.bestRank === null) return a.name.localeCompare(b.name);
    ...
    .map((e) => {
      const rankCell = e.partners
        ? e.partners
            .map(
              (p) =>
                `<div class="partner-rank"><span>${p.name}</span> <strong>${
                  p.match ? p.match.rank : "Unranked"
                }</strong>${p.match ? ` <em>${p.match.category}</em>` : ""}</div>`
            )
            .join("")
        : e.bestRank ?? "Unranked";
      return `
...
// Stamps the panel with the tournament it is editing, so two control panels open
// side by side (one per tournament) are never mistaken for each other.
async function applyTournamentIdentity() {
  const config = await api("/api/config");
  document.title = `${config.name} — Control Panel`;
  const heading = document.querySelector(".admin-header h1");
  if (heading) heading.textContent = `${config.name} — Control Panel`;
}
