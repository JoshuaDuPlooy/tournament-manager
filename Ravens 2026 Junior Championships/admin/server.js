...
// Each tournament folder (e.g. "Ravens 2026 Junior Championships/") holds its own
// tournament.config.json plus an admin/ + site/ pair. The admin/ and site/ code is
// identical across tournaments — only the config and the site/data/*.json differ —
// so a new tournament is just a copy of the folder with a new config.
const CONFIG_FILE = path.join(__dirname, "..", "tournament.config.json");

let tournamentConfig;
try {
  tournamentConfig = JSON.parse(require("fs").readFileSync(CONFIG_FILE, "utf-8"));
} catch (err) {
...
const PORT = process.env.PORT || tournamentConfig.adminPort || 4000;
...
// ---- Which tournament this control panel is editing ----
...
// Event names and ranking category headers are written differently in practice
// ("Men's 40+" on an entry form vs "MEN 40+" in the rankings sheet), so both sides are
// reduced to a common form before comparing: case, apostrophes and stray whitespace
// (including the line breaks entry forms often carry) are ignored, and the possessive
// "mens"/"womens" is treated as "men"/"women" — the rankings file itself mixes the two
// ("MENS 70+" alongside "MEN 60+").
...
// The rankings sheet lists singles categories only, so a doubles event never has a column
// of its own. Every category is returned instead and each partner is looked up individually
// client-side.
function isDoublesEvent(event) {
  return /\bdoubles\b/i.test(String(event ?? ""));
}
...
app.get("/api/rankings", async (req, res, next) => {
  try {
    ...
    worksheet.getRow(headerRowNumber).eachCell((cell, colNumber) => {
      ...
      if (!header) return;
      categories.push({ col: colNumber, header });
    });

    const doubles = isDoublesEvent(event);
    const wanted = normalizeCategory(event);
    const selected = doubles
      ? categories
      : categories.filter((c) => normalizeCategory(c.header) === wanted);

    if (selected.length === 0) {
      return res.status(400).json({
        error: `No ranking category matching "${event}" found in rankings.xlsx. Available categories: ${categories
          .map((c) => c.header)
          .join(", ")}`,
      });
    }
...
app.listen(PORT, () => {
  console.log(`${tournamentConfig.name} — control panel running at http://localhost:${PORT}`);
});
